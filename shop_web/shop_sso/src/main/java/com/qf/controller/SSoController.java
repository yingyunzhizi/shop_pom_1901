...
public class SSoController {
    ...
    public String login(User user, String returnUrl, Model model, HttpServletResponse response) {

        //因为如果你是注销后跳转的登录页面就没有之前的returnUrl传递过来,redirect就会报错,
     ...
     * 是否在登录状态
     *
     * @param loginToken:每一个页面的请求体里都带了cookie,@cookievalue,spring就帮我们获取了叫name的cookie, requiered=false,因为他默认true,如果请求中没有cookie就会报错,现在没有cookie也没问题
     * @param callback
     * @return
     */
    ...
    public String isLogin(@CookieValue(name = "login_token", required = false) String loginToken, String callback) {
        ...
        if (loginToken != null) {
            ...
            if (user != null) {
                //认证成功了,从redis中获取到了用户信息了
                //为什么一定要转化为JSON呢,不直接toString呢,是为了页面取值,直接data.nickname就可以取道值
    ...
    public String logout(
            @CookieValue(name = "login_token", required = false) String loginToken, HttpServletResponse response
    ) {
        if (loginToken != null) {
            //通过cookie删除redis中的loginToken
